...
import { getRecommendations } from "backend/data.tsx"
  ...
  function getAction() {
    if (type === "player") {
      ...
      return (
        ...
        >
          {always(() => {
            // this is a mindfuck and i blame uix, but it has to be this way for the always thing to work
            if (item.likes.val.has(userId))
              return (<div class="text-accent-500 fill-accent-500 stroke-accent-500 flex">
                <div class="font-semibold">{item.likes.val.size}</div><svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  class="size-6 ml-1"
      ...
      return (
        <button
          onclick={async () => {
            ...
            const recommendations = await getRecommendations(session?.$.queue.val);
            console.log(recommendations)
            // remove all elements from the recommended queue
            recommendations.forEach(() => {
              session?.$.recommendedQueue.val.pop()
            })
            recommendations.forEach((video) => {
              session?.$.recommendedQueue.val.push(video)
            })
          }}
