...
export default async function App() {
	...
	const Recommendations = ({ recommendedQueue }) => {
		return always(() => {
			// console.log('session', session)
			if (session.$.recommendedQueue.length === 0) {
				return null
			}
			...
			return (
				<div class="flex flex-col gap-3 mt-5">
					...
					<Queue items={session.$.recommendedQueue} type={'search'} code={code} />
				</div>
			)
		})
	}
			...
			<div class="mx-auto grid md:grid-cols-2 h-screen">
				...
				>
					...
					<div
						...
						<Recommendations recommendedQueue={session.$.recommendedQueue} />
